...
from app.services.faq_service import run_faq_job as _run_faq_job
from app.services.suggestion_service import (
    recompute_user_suggestions as _recompute_user_suggestions,
)
from app.services.thumbnail_service import run_thumbnail_job as _run_thumbnail_job
from app.services.video_frame_service import run_video_frame_job as _run_video_frame_job
...
async def run_thumbnail_job(ctx, job_id: str) -> None:
    async with async_session_factory() as db:
        await _run_thumbnail_job(db, UUID(job_id))


async def run_faq_job(ctx, job_id: str) -> None:
    async with async_session_factory() as db:
        await _run_faq_job(db, UUID(job_id))


async def run_video_frame_job(ctx, job_id: str) -> None:
    async with async_session_factory() as db:
        await _run_video_frame_job(db, UUID(job_id))
